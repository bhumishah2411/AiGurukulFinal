...
data = data.replace(/(\/\* Learn screen \*\/)[\s\S]*?(?=\/\* XP Toast \*\/)/, '$1\n' + newCSS + '\n\n    ');
...
data = data.replace(/<!-- ── Learn ─────────────────────────────────────────── -->[\s\S]*?(?=<script type="module">)/, refHTML + '\n\n  ');

// 3. Replace JS wiring
const jsBody = \`const GITA_CHAPTERS = [
...
const PANCHA_BOOKS = [
  ...
  {
    num:5, title:'अपरीक्षितकारकम्', roman:'Aparikshitakarakam', english:'Ill-Considered Action',
...
function buildGitaCards() {
  ...
  grid.innerHTML = GITA_CHAPTERS.map((c,i) => \`
    <div class="learn-card" style="animation-delay:\${i*0.04}s" onclick="toggleDetail(this)">
...
function buildPanchaCards() {
  ...
  grid.innerHTML = PANCHA_BOOKS.map((b,i) => \`
    <div class="learn-card" style="animation-delay:\${i*0.08}s;grid-column:span 1" onclick="toggleDetail(this)">
...
function buildAyurCards() {
  ...
  grid.innerHTML = AYURVEDA_HERBS.map((h,i) => \`
    <div class="learn-card" style="animation-delay:\${i*0.04}s;border-left:3px solid #3A9B8C" onclick="toggleDetail(this)">
...
function buildArthaCards() {
  ...
  grid.innerHTML = ARTHA_BOOKS.map((b,i) => \`
    <div class="learn-card" style="animation-delay:\${i*0.04}s;border-left:3px solid #C46B3A" onclick="toggleDetail(this)">
...
const extractedJS = "// ── Learn screen wiring ────────────────────────────────────\\n" +
  jsBody + \`
    ...
    // We override showScreen to handle the learn screen layout specifically
    ...
    showScreen = function(name) {
      ...
      if (name === 'learn') {
        ...
        screens[name].style.display = (name === 'response') ? 'block' : 'flex';
      }
      window.scrollTo(0, 0);
    };
...
data = data.replace(/\\/\\/ ── Learn screen wiring ────────────────────────────────────[\\s\\S]*?(?=\\/\\/ ── Init ───────────────────────────────────────────────────)/, extractedJS + '\\n    ');

fs.writeFileSync(indexFile, data);
console.log("Updated index.html directly from hardcoded strings!");
