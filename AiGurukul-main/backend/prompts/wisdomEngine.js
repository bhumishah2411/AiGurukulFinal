...
function parseWisdomResponse(raw) {
  const extract = (label, nextLabels) => {
    // Search for label, case-insensitive, with optional markdown and spaces/colons
    const labelRegex = new RegExp(`(?:\\*\\*|###)?\\s*${label}\\s*[:\\-]?\\s*(?:\\*\\*|###)?`, "i");
    ...
    for (const next of nextLabels) {
      const nextRegex = new RegExp(`(?:\\*\\*|###)?\\s*${next}\\s*[:\\-]?\\s*(?:\\*\\*|###)?`, "i");
      const nextMatch = raw.match(nextRegex);
  ...
  const parseAdviceLine = (prefix) => {
    // Look for prefix, then capture until the end of line OR until the next known prefix (AVOID, THINK, DO)
    // Look for prefix as a whole word, followed by optional punctuation
    const lineRegex = new RegExp(`(?:^|\\n|\\s)\\b${prefix}\\b\\s*[:\\-]?\\s*(.*?)(?=\\s*\\b(?:AVOID|THINK|DO)\\b\\s*[:\\-]|\\n|$)`, "i");
    const match = adviceRaw.match(lineRegex);
    if (match) return match[1].trim();

    // Fallback: if numbered list exists (1. ..., 2. ..., 3. ...)
    if (prefix === "DO") {
      const first = adviceRaw.match(/(?:^|\n)\s*1\.\s*(.+)/);
      return first ? first[1].trim() : "";
    }
    if (prefix === "AVOID") {
      const second = adviceRaw.match(/(?:^|\n)\s*2\.\s*(.+)/);
      return second ? second[1].trim() : "";
    }
    if (prefix === "THINK") {
      const third = adviceRaw.match(/(?:^|\n)\s*3\.\s*(.+)/);
      return third ? third[1].trim() : "";
    }
    return "";
  };
